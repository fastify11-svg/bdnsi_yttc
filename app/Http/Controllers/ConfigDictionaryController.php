...
class ConfigDictionaryController extends Controller
{
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('header_logo')) {
            $data['header_logo'] = \App\Lib\Image::storeFile($request->file('header_logo'), 'logos');
        }

        if ($request->hasFile('main_logo')) {
            $data['main_logo'] = \App\Lib\Image::storeFile($request->file('main_logo'), 'logos');
        }

        if ($request->hasFile('favicon')) {
            $data['favicon'] = \App\Lib\Image::storeFile($request->file('favicon'), 'logos');
        }
        ...
        \App\Models\ConfigDictionary::setMany($allConfigData);
