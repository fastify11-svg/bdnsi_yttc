...
export default function FooterLinkIndex({ footerLinks = [] }) {
    ...
    const handleConfirmDelete = () => {
        ...
        setProcessing(true);
        Inertia.delete(getUrl(`/admin/footer-link/${deleteModal.id}`), {
            onFinish: () => {
                setProcessing(false);
                setDeleteModal(null);
            }
    ...
    return (
        <AdminLayout title="Footer Links Management">
            <div className="space-y-6 max-w-7xl mx-auto">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div>
                        <h1 className="text-2xl font-black text-slate-900 tracking-tight">Footer Links</h1>
                        <p className="text-xs text-slate-500">Manage links that appear in the footer columns.</p>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8">
                    ...
                    <div className="lg:col-span-2 bg-white rounded-3xl shadow-xs border border-slate-200/80 overflow-hidden flex flex-col">
                        <div className="p-6 border-b border-slate-100">
                            <h2 className="text-lg font-black text-slate-900 tracking-tight">Existing Links</h2>
                        </div>
                        <div className="overflow-x-auto w-full">
                            <table className="w-full text-sm text-left text-slate-600">
                                <thead className="text-xs text-slate-500 uppercase bg-[#F8FAFC] border-b border-slate-200/80">
                                    <tr>
                                        <th className="px-6 py-4 font-extrabold text-slate-700">Sort Order</th>
                                        <th className="px-6 py-4 font-extrabold text-slate-700">Label</th>
                    ...
                    <div className="lg:col-span-1 bg-white rounded-3xl shadow-xs border border-slate-200/80 p-6 sm:p-8 space-y-6">
                        <div>
                            <p className="text-[10px] font-extrabold uppercase tracking-widest text-indigo-600">NEW LINK</p>
                        ...
                        <form onSubmit={handleCreateLink} className="space-y-4">
                            <div>
                                                ...
                                                />
                                                {errors.sort_order && <p className="text-xs text-rose-600 mt-1">{errors.sort_order}</p>}
                                            </div>
            ...
            {deleteModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <div className="relative w-full max-w-md bg-white rounded-3xl p-6 sm:p-8 shadow-2xl z-10 border border-slate-200/80 space-y-6">
                        ...
                        <div>
                            <h3 className="text-lg font-black text-slate-900">Delete Link?</h3>
