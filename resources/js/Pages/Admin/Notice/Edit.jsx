...
export default function Edit({ notice = {} }) {
    ...
    const { data, setData, post, processing, errors } = useForm({
        ...
        bn_details: notice.bn_details || '',
        ar_details: notice.ar_details || '',
        file_path: null,
    });

    const handleSubmit = (e) => {
        e.preventDefault();
        post(getUrl(`/admin/notice/${notice.id}`));
    };

    return (
        <AdminLayout title="Edit Notice">
            <div className="space-y-6 max-w-3xl mx-auto">
                <div className="flex justify-between items-center">
                    <div>
                        <h1 className="text-2xl font-black text-slate-900 tracking-tight">Edit Notice</h1>
                        <p className="text-xs text-slate-500">Update existing notice details or multi-language content.</p>
                    </div>
                ...
                <div className="bg-white rounded-3xl p-6 sm:p-10 shadow-xs border border-slate-200/80 space-y-6">
                    <form onSubmit={handleSubmit} className="space-y-5 text-xs">
                        ...
                        <div>
                            ...
                            {notice.file_url && (
                                <a href={notice.file_url} target="_blank" rel="noreferrer" className="text-indigo-600 text-xs font-bold block mb-2 underline">View Current File</a>
                            )}
                            <input
                                type="file"
