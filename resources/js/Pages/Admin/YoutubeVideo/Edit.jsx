...
export default function Edit({ video = {} }) {
    ...
    const { data, setData, put, processing, errors } = useForm({
        title: video.title || '',
        link: video.link || (video.video_id ? `https://www.youtube.com/watch?v=${video.video_id}` : ''),
        description: video.description || '',
    });

    const handleSubmit = (e) => {
        e.preventDefault();
        put(getUrl(`/admin/youtube-video/${video.id}`));
    };

    return (
        <AdminLayout title="Edit YouTube Video">
            <div className="space-y-6 max-w-3xl mx-auto">
                <div className="flex justify-between items-center">
                    <div>
                        <h1 className="text-2xl font-black text-slate-900 tracking-tight">Edit YouTube Video</h1>
                        <p className="text-xs text-slate-500">Update video title, link, or description.</p>
                    </div>
