...
foreach ($sessions as $session) {
    ...
    if (preg_match('/^(\d{4})\s*to\s*(\d{4})$/', $name, $match)) {
        ...
        if (isset($monthsMap[$match[1]]) && isset($monthsMap[$match[2]])) {
            $startMonth = $monthsMap[$match[1]];
            $endMonth = $monthsMap[$match[2]];
